...
function renderContent(content: string): string {
  return content
    ...
    // Code blocks
    .replace(/```(\w+)?\n([\s\S]*?)```/gim, '<pre class="bg-background border border-border rounded-lg p-4 overflow-x-auto my-6"><code class="text-sm font-mono text-foreground">$2</code></pre>')
    // Inline code
    .replace(/`([^`]+)`/gim, '<code class="bg-background px-1.5 py-0.5 rounded text-sm font-mono text-accent">$1</code>')
    // Links
    ...
    // Blockquotes
    .replace(/^> (.*$)/gim, '<blockquote class="border-l-4 border-accent pl-4 my-4 text-muted italic">$1</blockquote>')
    // Unordered lists
    .replace(/^\- (.*$)/gim, '<li class="ml-6 list-disc text-muted">$1</li>')
    // Ordered lists
    .replace(/^\d+\. (.*$)/gim, '<li class="ml-6 list-decimal text-muted">$1</li>')
    // Horizontal rule
    .replace(/^---$/gim, '<hr class="border-border my-8" />')
    // Paragraphs (wrap remaining text)
    .replace(/^(?!<[hlpbuio]|<\/|<pre|<block|<img|<hr)(.+)$/gim, '<p class="text-muted leading-relaxed mb-4">$1</p>')
    // Clean up empty paragraphs
    .replace(/<p class="text-muted leading-relaxed mb-4"><\/p>/gim, '')
